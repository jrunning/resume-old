...
class ResumeRenderer
  attr_reader :content, :template, :env
...
TEMPLATE: |- # TODO Add hResume microformat: http://microformats.org/wiki/hresume
  ...
  <html>
    <head>
      <title><%= details['name'] %> &mdash; <%= details['email'] %> &mdash; Resume</title>
    <% unless env == 'pdf' %>
      <link rel="stylesheet"
        href="http://fonts.googleapis.com/css?family=Lato:300,300italic,400,400italic,900italic">
    <% end %>
    <% unless env == 'production' || env == 'pdf' %>
    ...
    <% if @env == 'pdf' %>
      <footer>Cheerfully generated by <cite>resume.rb</cite>:
        <a href="http://github.com/jrunning/Resume">http://github.com/jrunning/Resume</a>
      </footer>
    <% else %>
